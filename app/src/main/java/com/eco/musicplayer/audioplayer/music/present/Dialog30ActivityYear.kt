        ...
        runOnUiThread {
            ...
            if (!offers.isNullOrEmpty()) {
                ...
                if (pricingPhases.isNotEmpty()) {
                    ...
                    val paidPhase = pricingPhases.firstOrNull { it.billingCycleCount != 0 } ?: pricingPhases.last()
                    
                    // Parse billing period để hiển thị đẹp hơn
                    ...
                    Log.d(TAG, "=== UI UPDATE ===")
                    Log.d(TAG, "Trial: ${trialPhase.formattedPrice} for ${trialPhase.billingPeriod}")
                    Log.d(TAG, "Paid: ${paidPhase.formattedPrice} for ${paidPhase.billingPeriod}")
...
//      P1Y -> "1 year"
    ...
    private fun findOfferById(offers: List<ProductDetails.SubscriptionOfferDetails>, offerId: String): ProductDetails.SubscriptionOfferDetails? {
